...
(function (factory) {
	...
	$.jstree.plugins.columns = function (options, parent) {		
		...
		var replaceKeyValues = function (object, key, mainInstance, newInstance, textKey) {			
			...
			if (object[newKey] && object[newKey].text && textKey == "column1text") {
				object[newKey].text = object[newKey].original.code == null ? "" : object[newKey].original.code;
			}

			if (object[newKey] && object[newKey].text && textKey == "column2text") {
				object[newKey].text = object[newKey].original.disposalCode == null ? "" : object[newKey].original.disposalCode;
			}
